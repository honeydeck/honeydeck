...
`magic` is forward-only; backward navigation into a slide with `transition: magic` falls back to `fade`. Magic transition matching is explicit: only elements with the same `data-magic-id` on the outgoing and incoming slides move. Previous-only tagged elements fade out, incoming-only tagged elements fade in, and untagged slide content crossfades with the slide layers. Honeydeck performs no automatic DOM diffing and no text/tag heuristic matching. Runtime magic keeps both slide layers mounted, measures tagged elements with `getBoundingClientRect()`, hides tagged originals, animates fixed-position overlay clones with copied computed styles, accounts for the active slide scale, and restores originals after cleanup. Layout chrome outside slide content switches immediately and does not participate unless explicitly tagged inside slide content.
