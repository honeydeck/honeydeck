...
describe("init templates", () => {
	...
	it("documents the default layout, transitions, and theme swap options", () => {
		...
		assert.match(styles, /@honeydeck\/honeydeck\/theme\.css/);
		assert.match(styles, /@honeydeck\/honeydeck\/themes\/bee\.css/);
	});
	...
	it("pins generated projects to the current Honeydeck release line", () => {
		...
		assert.equal(
			generatedPackage.dependencies["@honeydeck/honeydeck"],
			`^${packageJson.version}`,
		);
	});
