...
- `deck.mdx` demonstrates named slide transitions, including built-in `slide-left`, a custom CSS transition named `honey-spin` defined in `styles.css`, and `magic` transitions for explicit `data-magic-id` move/fade behavior.
