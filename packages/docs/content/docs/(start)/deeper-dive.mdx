...
Use `--force` when the browser reports a pre-bundled dependency export error; see [Troubleshooting](/docs/troubleshooting).
...
| Scaling | CSS `zoom` from the configured aspect ratio |
