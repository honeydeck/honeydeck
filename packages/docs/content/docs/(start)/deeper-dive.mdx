...
For more detail, see [Slides](/docs/slides).
...
For the full reference, see [Configuration](/docs/configuration) and [Transitions](/docs/transitions).
...
```mdx
import { Reveal, RevealWith, RevealGroup, TimelineSteps, BrowserFrame, Notes } from '@honeydeck/runtime'
```
...
See [Steps and reveals](/docs/steps-and-reveals) for examples.
...
Standard iframe attributes such as `allow`, `sandbox`, `loading`, and `referrerPolicy` are forwarded. See [Components](/docs/components) for props.
...
See [Navigation](/docs/navigation), [Mobile and touch](/docs/mobile), [Presenter mode](/docs/presenter-mode), and [PDF export](/docs/pdf-export).

## Theme system

Honeydeck uses CSS custom properties with relative `oklch` color derivations. All colors derive from `--honeydeck-primary`.

`@honeydeck/runtime/theme.css` is the clean black/white/grey default. Override tokens in your CSS:
...
See [Customization](/docs/customization) for theme and layout customization.
...
Zero config still works: built-in defaults apply without imports. See [Customization](/docs/customization).
...
See [Skills](/docs/skills) for installation options and bundled skill details.
Coming from Slidev? See the [Slidev migration guide](/docs/slidev-migration).
