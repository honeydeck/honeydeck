...
| `↓` / `s` | Next slide |
| `↑` / `w` | Previous slide |
| `h` | In overview mode, show or hide hidden slides in the grid |
...
Slides with `hidden: true` are skipped in both directions, so navigation lands on the nearest non-hidden slide. See [Hidden Slides](/docs/slides#hidden-slides).
...
Slide numbering counts every slide, including slides marked `hidden: true`. Displayed numbers therefore stay in sync with the URL even when a deck contains hidden slides, and a hidden slide shows its own deck number while it is current.
...
- Hidden slides are left out of the grid; press `h` or use the `Show hidden` header button to list them, then click or press Enter to jump to one. Listed hidden thumbnails carry a `Hidden` badge.
- Opening overview while the current slide is hidden lists hidden slides right away.
...
- Slides marked `hidden: true` count as slides and keep their number, so URLs stay stable no matter how many slides are hidden.
