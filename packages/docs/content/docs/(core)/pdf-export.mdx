...
Slides with `hidden: true` are backup material for live navigation, so PDF export skips them in both `--steps final` and `--steps all`. Page counts and progress output cover the exported pages only. Exporting a deck whose slides are all hidden fails with a message that no slides were found for export.
