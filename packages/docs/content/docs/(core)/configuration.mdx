...
For built-ins, magic `data-magic-id` matching, custom CSS transitions, duration, and easing, see [Transitions](/docs/transitions).
...
Use `layouts: "./layouts"` for custom layouts or `layouts: "@honeydeck/runtime/layouts/bee"` for the playful Bee layouts.
