...
- `magic` — forward-only FLIP transition for explicitly tagged elements
...
Use `transition: magic` to move explicitly tagged elements between slides:
...
Matching uses only `data-magic-id`:

- same id on both slides → element moves
- id only on previous slide → element fades out
- id only on incoming slide → element fades in
- untagged content crossfades with slide layers

Honeydeck does not do automatic DOM diffing or text matching. Backward navigation with `magic` falls back to `fade`.
