...
- Hidden slides count as slides and keep their number, so `/#/7/0` always points at the same slide. The navigation bar number, the `showSlideNumbers` overlay, and the presenter counter (current number and total) all use that numbering.
