...
Body content follows the title.
```

Components used in the title must be imported at the top of the slide. They resolve with the same rules as components used in the body. Inline Markdown emphasis, code, and links work as usual:

```mdx
# Honeydeck `v1.0` is **here**
```

Timeline-consuming components such as `<Reveal>` are not assigned steps when used in a title.
