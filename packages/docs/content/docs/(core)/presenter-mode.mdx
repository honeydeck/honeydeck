...
The `Next` preview follows timeline navigation, so it skips slides marked `hidden: true`. While the current slide is hidden, the slide counter appends `Hidden`. The counter uses deck slide numbering, so both the current number and the total count hidden slides.
...
- Press `h` to list hidden slides in the grid, then jump to one — this is how you reach a backup slide during a talk.
