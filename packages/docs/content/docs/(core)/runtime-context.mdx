...
- `currentSlide.index` is 1-based and matches the slide number in the URL. Slides marked `hidden: true` count in this numbering.
...
- `currentSlide.layoutProps` is the parsed slide frontmatter passed to the layout as `frontmatter`, including `hidden` when the slide sets it.
...
Honeydeck renders each slide at logical canvas dimensions, then scales the full slide with CSS `zoom` to fit the current viewport or preview surface. Scaling with `zoom` lays the slide out at its rendered size, so text and operating system emoji glyphs stay crisp and uncropped in every browser. Normal HTML and CSS content does not need to account for this scale.

Use `useSlideScale()` only when a component or layout needs the current scale factor applied to the logical slide canvas.

## Scale-aware canvas and WebGL components

Some canvas and WebGL libraries measure their container with `getBoundingClientRect()` and write that measured size back into the canvas style. Because `getBoundingClientRect()` reports scaled pixels, those libraries can accidentally apply Honeydeck's slide scale twice and render smaller than their wrapper.
