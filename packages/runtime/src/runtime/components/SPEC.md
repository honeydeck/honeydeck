...
`@honeydeck/runtime/components` exports generic Honeydeck-token-based button primitives and class recipes for runtime chrome, runtime reference pages, and docs controls.

```tsx
import { Button, buttonPrimaryClass, iconButtonClass } from '@honeydeck/runtime/components'
```
...
```mdx
import { Keyboard } from '@honeydeck/runtime'
...
```mdx
import { BrowserFrame } from '@honeydeck/runtime'
