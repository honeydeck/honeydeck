...
Shows generated documentation for each public built-in component exported from `@honeydeck/runtime/components`, discovered from the component barrel at build time. Unlike the layouts tab, components are primarily documented with prose and usage examples rather than visual previews:
