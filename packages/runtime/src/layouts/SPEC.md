...
Styling is provided only by explicit CSS imports. Honeydeck does not auto-inject Tailwind, `@honeydeck/runtime/theme.css`, or custom theme CSS. The starter project imports `./styles.css` from `deck.mdx`; `styles.css` imports Tailwind and `@honeydeck/runtime/theme.css`. Without that import, slides still render with built-in layouts, but mostly with browser/default styling. User imports override via cascade.
...
|--------------|---------|
| `@honeydeck/runtime/theme.css` | Preferred base theme import; clean black/white/grey defaults |
| `@honeydeck/runtime/themes/base.css` | Package export alias for the base theme |
| `@honeydeck/runtime/themes/clean.css` | Optional clean visual style layer |
| `@honeydeck/runtime/themes/bee.css` | Bee theme layer for the original playful palette |

Use `@honeydeck/runtime/layouts` for clean default layouts. Use `@honeydeck/runtime/layouts/bee` together with `@honeydeck/runtime/themes/bee.css` for the Bee visual style.
...
Honeydeck provides eight clean default layouts. Honeydeck also ships `@honeydeck/runtime/layouts/clean` as a named clean kit import path and `@honeydeck/runtime/layouts/bee` for the playful Bee layouts:
