...
export function resolveLayoutMapPath({
	...
	if (specifier === "@honeydeck/runtime/layouts") {
		return resolve(packageRoot, "src/layouts/index.ts");
	}

	if (specifier === "@honeydeck/runtime/layouts/clean") {
		return resolve(packageRoot, "src/layouts/clean/index.ts");
	}

	if (specifier === "@honeydeck/runtime/layouts/bee") {
...
export function crawlLayoutDemos(
	...
	if (!mapPath) {
		context.warnings.push(
			`Could not resolve layout map "${options.layoutSpecifier || "@honeydeck/runtime/layouts"}" for demo discovery.`,
		);
		return {
			mapPath: null,
			demos: [],
...
function resolveImportedModule(
	...
	if (specifier.startsWith("@honeydeck/runtime/layouts/clean/")) {
		const layoutName = specifier.slice(
			"@honeydeck/runtime/layouts/clean/".length,
		);
		const builtInPath =
			layoutName === "Image"
	...
	if (specifier.startsWith("@honeydeck/runtime/layouts/bee/")) {
		const layoutName = specifier.slice(
			"@honeydeck/runtime/layouts/bee/".length,
		);
		const builtInPath =
			layoutName === "Image"
	...
	if (specifier.startsWith("@honeydeck/runtime/layouts/")) {
		const layoutName = specifier.slice("@honeydeck/runtime/layouts/".length);
		const builtInPath =
			layoutName === "Image"
				? resolve(packageRoot, "src/layouts/clean/Image/Image.tsx")
				: resolve(packageRoot, `src/layouts/clean/${layoutName}.tsx`);
...
function toPublicSpecifier({
	...
	if (!relToBuiltIn.startsWith("..") && !isAbsolute(relToBuiltIn)) {
		...
		if (withoutExt === "index") return "@honeydeck/runtime/layouts";
		if (withoutExt === "clean/index") return "@honeydeck/runtime/layouts/clean";
		if (withoutExt === "bee/index") return "@honeydeck/runtime/layouts/bee";
		...
		if (withoutExt.startsWith("clean/")) {
			...
			if (
				originalSpecifier.startsWith("@honeydeck/runtime/layouts/clean") ||
				mapRel.startsWith("clean/")
			) {
				return `@honeydeck/runtime/layouts/clean/${cleanSubpath}`;
			}
			return `@honeydeck/runtime/layouts/${cleanSubpath}`;
		}
		...
		if (withoutExt.startsWith("bee/")) {
			...
			return `@honeydeck/runtime/layouts/bee/${beeSubpath}`;
		}

		return `@honeydeck/runtime/layouts/bee/${layoutSubpath}`;
	}
