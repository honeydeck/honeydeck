...
export const demo = {
  mdx: "import { Left, Right } from '@honeydeck/runtime/layouts/clean/TwoCol'\n\n# Clean TwoCol Demo",
}
`,
	);
	writeFileSync(
...
export const demo = { mdx: "import Hive from '@honeydeck/runtime/layouts/bee/Hive'\n\n# Bee Hive Demo" }
`,
	);
}
...
`,
		...
		assert.equal(
			...
			"@honeydeck/runtime/layouts/TwoCol",
		);
		assert.equal(
	...
	it("discovers demos from named imports re-exported by layout barrels", () => {
		...
		writeFileSync(
			join(root, "layouts.ts"),
			`
import { DefaultLayout as CleanDefaultLayout } from '@honeydeck/runtime/layouts'
...
`,
		...
		assert.equal(
			...
			"@honeydeck/runtime/layouts/Default",
		);
	});
...
`,
	...
	it("discovers demos from a dedicated clean layout map fixture", () => {
		...
		const result = crawlLayoutDemos({
			entryPath: join(root, "deck.mdx"),
			packageRoot: root,
			layoutSpecifier: "@honeydeck/runtime/layouts/clean",
		});
		...
		assert.equal(
			result.demos.find((demo) => demo.layoutName === "TwoCol")
				?.publicModuleSpecifier,
			"@honeydeck/runtime/layouts/clean/TwoCol",
		);
		assert.match(
			result.demos.find((demo) => demo.layoutName === "TwoCol")?.demoMetadata
				?.mdx ?? "",
			/import \{ Left, Right \} from '@honeydeck\/runtime\/layouts\/clean\/TwoCol'/,
		);
		assert.deepEqual(
	...
	it("discovers demos from a dedicated Bee layout map fixture", () => {
		...
		const result = crawlLayoutDemos({
			entryPath: join(root, "deck.mdx"),
			packageRoot: root,
			layoutSpecifier: "@honeydeck/runtime/layouts/bee",
		});
		...
		assert.equal(
			result.demos.find((demo) => demo.layoutName === "Hive")
				?.publicModuleSpecifier,
			"@honeydeck/runtime/layouts/bee/Hive",
		);
		assert.match(
