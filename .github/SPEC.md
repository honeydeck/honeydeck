...
- dependency installation from the root `pnpm-lock.yaml`
...
- commit the release version metadata (`packages/honeydeck/package.json` and root `pnpm-lock.yaml`) back to `main` after publication using a GitHub App installation token that can bypass the `main` ruleset and a non-Conventional Commit `[skip ci]` message so the release commit does not trigger another release
